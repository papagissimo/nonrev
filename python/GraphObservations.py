...
Two variants used to be computed per flight-instance, t1Old and t1New:
- t1Old: actual (binary-search) cabin values only, missing = 0 - this is
  the estimate exactly as it's always worked, unchanged.
- t1New (RETIRED 2026-09-14, along with all glance/cheap-derived
  estimation - FloorEstimates.py itself is gone): same, but where an
  actual value is missing and a cheap-side floor glance was present,
  substituted the real data-driven estimate from FloorEstimates.py
  instead of treating the cabin as 0.
t1Old alone remains below - his own historical name for it, kept rather
than renamed to avoid touching every frontend reference for a field
that's itself a separately-decided retirement target (his call: wire
this graph to the SAME curve-slide estimator SeatLoggingDialog/
T1Estimator.py already use, instead of keeping this old two-point
method around at all) - not done here, since that's a materially bigger
change than removing the now-orphaned glance comparison this docstring
used to describe.
"""
...
def get_flight_points(conn, org, dest, days_of_week, date_from, date_to):
    ...
    Each point: {flightDate, dow, depTimeMinutes, depTimeDisplay,
                 flightNumber, t1Old, confidenceHalfRange, numReadings}
    t1Old is the estimate exactly as it's always worked (actual cabin
    values only, missing = 0) - see module docstring for the retired
    t1New glance-substitution variant.
    ...
    where = ["org = ?", "dest = ?", excluded_date_where_clause(), not_seat_map_only_where_clause()]
    params = [org, dest]
    if date_from:
    ...
    for flight_date, date_rows in by_date.items():
        ...
        for (f_date, check_ts, dep_time, flight_number, hrs, y, c_plus, first_ps, d1) in date_rows:
            if dep_time is None:
    ...
    def resolve_cabin(actual):
        """One cabin's contribution to a reading's ttl: actual value if
        present, else 0. No glance/cheap fallback - retired 2026-09-14
        along with all glance-derived estimation (see module docstring's
        note on the old t1New variant)."""
    ...
    for (flight_date, dep_minutes), obs_list in groups.items():
        ...
        for obs in obs_list:
            ...
            y_old = resolve_cabin(as_int(obs['y']))
            cp_old = resolve_cabin(as_int(obs['cPlus']))
            fp_old = resolve_cabin(as_int(obs['firstOrPS']))
            d1_old = resolve_cabin(as_int(obs['d1']))
